...
- Regra em `api/app/services/spike_alert_service.py`: ganho de views das últimas ~24h (snapshot mais próximo de 24h atrás, tolerância 12–48h) >= multiplicador × média diária dos ~7 dias anteriores (span mínimo 3 dias; piso de 100 views/dia na média; cooldown 24h/canal). Chamado no loop de canais do `run_sync` via `safe_check_channel` (nunca derruba o sync). **Baseline <= 0 (canal perdeu views na janela, ex.: vídeos apagados) NÃO dispara** — sem isso o piso de 100 virava divisor fictício e gerava alerta falso tipo "2457x". A mensagem exibe a média efetivamente usada no ratio (`metadata.baseline_used`; `baseline_daily` é o valor cru).
...
- **Vídeos por canal** (`GET /api/analytics/videos-by-channel`): lista canais paginada (máx 20/página) com todos os vídeos monitorados e séries temporais de VPD e views. Busca canais → vídeos → snapshots em 3 queries (sem N+1). UI: rota própria `/analytics/videos` (`web/app/analytics/videos/page.tsx` → `VideosByChannelView.tsx`), linkada pelo grupo retrátil "Analytics" do menu lateral. Os gráficos de cada vídeo ficam SEMPRE abertos — o único nível de recolher é o canal (cabeçalho do grupo). Antes era aba interna do `AnalyticsView` com expansão por vídeo; as duas coisas foram removidas. Deep-links: `?q=` preenche a busca (casa nome de canal OU título de vídeo) e `?highlight=<youtube_video_id>` destaca e rola até o vídeo — usados pelos botões "gráficos do vídeo"/"vídeos" das abas do Dashboard. Filtro **"Vídeos: Todos / Só os que estão subindo"** (`only_rising=true` no endpoint): reusa `_videos_accelerating_rows` (mesma regra do card "Vídeos acelerando"), poda os vídeos E tira da paginação os canais que ficariam vazios, então `total` continua batendo com o exibido.
...
- **Highlights do Dashboard** (`GET /api/analytics/highlights?kind=heating|promising|saturated|videos_accelerating&status=active&limit=N`): lista COMPACTA por trás de cada contador do `/overview` — sem séries temporais nem `channel_summary`, só o que cabe numa linha de tabela. Usa os mesmos filtros do `/overview` (mesmo `status`, mesmo sinal do último snapshot, mesma regra de vídeo acelerando via `_videos_accelerating_rows`, que o contador também usa), então o número do card sempre bate com a lista. `total` vem antes do corte por `limit` (máx 200). UI: `web/app/DashboardHighlights.tsx` — os 4 cards do Dashboard viraram botões que abrem a tabela logo abaixo; "abrir no Analytics" usa o deep-link `/analytics?signal=<sinal>` (lido no `AnalyticsView`, junto com `?q=`).
